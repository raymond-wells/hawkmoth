...
class _AutoBaseDirective(SphinxDirective):
    ...
    def __transform(self, lines):
        transform = self.options.get(
            "transform", self.env.config.hawkmoth_transform_default
        )
    ...
    def __get_docstrings(self, viewlist, filename):
        ...
        for docstrings in root.walk(
            recurse=False,
            filter_types=self._docstring_types,
            filter_names=self._get_names(),
        ):
            for docstr in docstrings.walk(filter_names=self._get_members()):
                lineoffset = docstr.get_line() - 1
                lines = docstr.get_docstring(
                    transform=lambda lines: self.__transform(lines)
                )
                for line in lines:
        ...
        for pattern in self.arguments:
            ...
            for filename in filenames:
                if os.path.isfile(filename):
                    ...
                    self.logger.warning(
                        f'Path "{filename}" matching pattern "{pattern}" is not a file.',  # noqa: E501
                        location=(self.env.docname, self.lineno),
                    )
...
class _AutoSymbolDirective(_AutoBaseDirective):
    ...
    def _get_filenames(self):
        ...
        if not filename:
            self.logger.warning(
                ":file: option missing.", location=(self.env.docname, self.lineno)
            )
            return []
